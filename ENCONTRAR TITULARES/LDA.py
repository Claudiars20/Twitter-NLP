...
def TopicModeling( topic, cte ):
    ...
    ldamodel = gensim.models.ldamodel.LdaModel(corpus, num_topics = NUM, id2word=dicc, passes = 100, iterations = 100000, chunksize = 10000, eval_every = None, random_state = 0)
    ...
    df_dominant_topic.drop(df_dominant_topic.loc[df_dominant_topic['Porcentaje de Contribución'] <= 0.92].index, inplace=True) #0.92
...
def Csv_With_TopicModeling( topic , cte ):
...
def Main( topic, n_topics ):
    ...
    # CONSTANTE PARA AGRUPACION LDA Y AGRUPAR TERMINOS
    url = "PONDERADO/PONDERADO CON CONTEXTO/" + topic + ".csv"
    df = pd.read_csv(url)
    tamanio = len(df['Name'].values) // 50
    cte = 15 if tamanio == 0 else tamanio * 15


    Keywords_WithWords(topic)
    TopicModeling(topic, cte)
    Csv_With_TopicModeling(topic, cte)
